...
indirs <- lapply(samples, function(x) file.path(data_dir, x, "MT", "cellr_True", paste0(x,"_200")))
names <- lapply(samples, function(x) file.path('mgatk',paste0(x, '.wrap')))
seurat_rds <- lapply(samples, function(x) file.path(data_dir, x, "MT", "cellr_True", paste0(x,"_200"), 'mgatk', paste0(x,'.wrap.rds')))
