...
rule knn_init:
    input:
        af ="{outdir}/multiplex/clones_init/donor{d}/af.tsv"
    output:
        cells = "{outdir}/clones/variants_init/knn/kparam_{kparam}/donor{d}/cells_meta.tsv",
        fig = "{outdir}/clones/variants_init/knn/kparam_{kparam}/donor{d}/donor{d}.variants.labels.png"
...
rule knn_init_concat:
    input:
        cells = expand("{{outdir}}/clones/variants_init/knn/kparam_{{kparam}}/donor{d}/cells_meta.tsv",
                        d=np.arange(config["N_DONORS"])),
    output:
        cells_all = "{outdir}/clones/variants_init/knn/kparam_{kparam}/cells_meta.tsv"
...
rule vireo_init:
    ...
    output:
        ...
        "{outdir}/clones/variants_init/vireo/nclones{nclones}/donor{d}.labels.png",#, category="lineage", subcategory="donorMGATK"),
        "{outdir}/clones/variants_init/vireo/nclones{nclones}/donor{d}.variants.labels.png",# category="lineage", subcategory="donorMGATK"),
        "{outdir}/clones/variants_init/vireo/nclones{nclones}/donor{d}_cells_meta.tsv",
    params:
        INDIR = lambda wildcards, input: dirname(input[0]),
        OUTDIR = lambda wildcards, output: dirname(output[0]),
...
rule vireo_init_concat:
    input:
        expand("{{output}}/clones/variants_init/vireo/nclones{{nclones}}/donor{d}_cells_meta.tsv",
            d = np.arange(config["N_DONORS"])),
    output:
        "{outdir}/clones/variants_init/vireo/nclones{nclones}/cells_meta.tsv",
    run:
        all = []
        for i in input:
            all.append(pd.read_csv(i, sep='\t'))
