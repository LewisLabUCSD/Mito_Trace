...
rule needlestack_all:
    ...
    #     #bed_in = config["merged_peaks_needle_f"]  # "{outdir}/bed/peaks.bed"
    # output:
    #     vcf = "{outdir}/aggregate/needlestack/all_variants.vcf"
    # params:
    #     bam_indir = lambda wildcards, input: dirname(input.bam_in[0]),
    #     ref = config["ref_fa"],
    #     min_dp = 15
    # shell:
    #     "nextflow run iarcbioinfo/needlestack --input_bams {params.bam_indir}/ --ref {params.ref} --min_dp {params.min_dp} --output_vcf {output.vcf}"
