...
def get_gaussian_thresh(df, prob_thresh=0.9, to_plot=False, f_save=""):
    cell_depth = np.log10(df.groupby("Cell")["Coverage"].sum())
    ...
    cells_to_keep = np.where(probs[:, max_clust] > prob_thresh)
    cell_labels = cell_inds[cells_to_keep]
    print(f"Initial cells: {len(cell_inds)}\n Number of cells passed: {len(cell_labels)}")
    if to_plot:
        f, ax = plt.subplots()
        sns.distplot(cell_depth)
        sns.distplot(cell_depth.loc[cell_labels], ax=ax, color='g')
        plt.title(
            "Cell total coverage across MT. Filtering out low coverage")
        if not (f_save == ""):
            plt.savefig(f_save.replace(".png", "") + ".png")
    return cell_labels, gm


def filter_positions_cells(sc_coverage, topN=500, min_cells=100, min_reads=100):
    ...
    cell_gauss, _ = get_gaussian_thresh(sc_coverage, prob_thresh=0.9, to_plot=False, f_save="")
    print('cell gauss', len(cell_gauss))
...
def load_and_filter_nt_pileups(concat_dir, cell_inds, nt_pos,
    ...
    for n in ["A", "C", "G", "T"]:
        ...
        if len(df.columns)>4:
            ...
            df.columns = ["Position", "Cell", "Coverage", "BQ"]
        df["Cell"] = df["Cell"].apply(lambda x: x.replace(".bam", ""))
        df = df[df["Cell"].isin(cell_inds)]
        df = df[df["Position"].isin(nt_pos)]
        if out_d is None or name is None:
            print("Not saving, need an out_d and name. Only returning nt_pileup.")
        else:
    ...
    if incl_cov:
        df = load_sc_pileup(os.path.join(concat_dir, f"*.coverage.strands.txt.gz"))
        df = df[df["Cell"].isin(cell_inds)]
        df = df[df["Position"].isin(nt_pos)]
        if out_d is None or name is None:
            print("Not saving, need an out_d and name. Only returning nt_pileup.")
...
def calculate_af(scpileup_dir, AF_F, mt_ref_fa, name="",
    ...
    af_by_cell = cov_ad.pivot(index="Cell", columns="ID",
                              values="AF").fillna(0)
    sns.heatmap(np.sqrt(af_by_cell), vmin=0, vmax=0.3, ax=ax[0, 0])
    ax[0].set_title("After filtering for counts")
    ...
    sns.heatmap(np.sqrt(af_by_cell), vmin=0, vmax=0.3, ax=ax[1, 0])
    ...
    sns.heatmap(np.log2(depth_by_cell + 1), ax=ax[1, 1])
    sns.heatmap(np.log2(ad_by_cell + 1), ax=ax[1, 2])
    ...
    sns.heatmap(np.sqrt(af_by_cell), vmin=0, vmax=0.3, ax=ax[2,0])
    ...
    depth_by_cell = cov_ad[(cov_ad["ID"].isin(af_by_cell.columns)) & (
        cov_ad["Cell"].isin(af_by_cell.index))].pivot(index="Cell",
                                                      columns="ID",
                                                      values="Coverage").fillna(0)
    sns.heatmap(np.log2(depth_by_cell + 1), ax=ax[2, 1])
    sns.heatmap(np.log2(ad_by_cell + 1), ax=ax[2, 2])
    ...
    sns.heatmap(np.sqrt(af_by_cell), vmin=0, vmax=0.3, ax=ax[3,0])
    ax[3,0].set_title("After filtering BQ averages")
    sns.heatmap(np.log2(depth_by_cell + 1), ax=ax[3, 1])
    sns.heatmap(np.log2(ad_by_cell + 1), ax=ax[3, 2])
    ...
    if AF_F is not None:
        ...
        bq_by_cell.to_csv(AF_F.replace('.csv', '') + ".bq.tsv", sep='\t')
        ad_by_cell.to_csv(AF_F.replace('.csv', '') + ".AD.tsv", sep='\t')
        depth_by_cell.to_csv(AF_F.replace('.csv', '') + ".DP.tsv", sep='\t')
        ...
        f_heat.suptitle(
            "Sqrt(allele freq), log2(allele count+1), log2(depth count+1)")
        helper_save(join(out_d, 'heatmap.png'), f=f_heat)
        return
    else:
