...
def run_vireo(ad, dp, k, out_f=None, plot_qc=False, n_cores=2,
              n_initials=50):
    # #_ELBO_mat = []
    # _models_all = []
    # _elbo_temp = []
    ...
    _models_all = Parallel(n_jobs=n_cores)(
        delayed(run_single)((ad, dp), k) for i in range(n_initials))
...
def plot_vireo_out(modelCA, out_f, to_sqrt=False, labels=None,
    ...
    plt.subplots(figsize=(7, 4), dpi=100, nrows=2, ncols=2)
    plt.subplot(1, 2, 1)
...
def run_elbo(ad, dp, out_f, sample_colors,
    ...
    for k in n_clone_list:
        ...
        if os.path.exists(curr_clone_model_f) and (not rerun_model):
            ...
            modelCA = run_vireo(ad, dp, k, out_f + f".clones{k}.modelCA.p")
