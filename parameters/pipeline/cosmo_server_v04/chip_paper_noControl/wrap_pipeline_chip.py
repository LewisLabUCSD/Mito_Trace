...
cfg_f = join(ROOT_DIR, "parameters/pipeline/cosmo_server_v04", "chip_paper_noControl", "wrap_pipeline_chip.yaml")
...
if exists(join(ROOT_DIR, ".envs", "email.yaml")):
    email = read_config_file(join(ROOT_DIR, ".envs", "email.yaml"))["email"]
else:
    email=None
