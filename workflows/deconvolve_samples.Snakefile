...
rule all:
    input: expand("data/{name}_cellSNP_minC{minC}_minAF{minAF}", name=config["samples"], minC=minC, minAF=minAF),
           ...
           expand("results/chrM/{name}_cellSNP_minC{mt_minC}_minAF{mt_minAF}/lineage_chrM.ipynb", name=config["samples"], mt_minC=mt_minC,mt_minAF=mt_minAF),
           expand("data/chrM/pseudo/minC{mt_minC}_minAF{mt_minAF}/numC{num_cells}_isprop{is_prop}/pseudo.ipynb", num_cells=num_cells, is_prop=is_prop, mt_minC=mt_minC, mt_minAF=mt_minAF)
...
rule vireo_chrM:
    ...
    #"data/chrM/cellSNP_minC{mt_minC}_minAF{mt_minAF}/lineage_chrM.ipynb"
    output: "results/chrM/{name}_cellSNP_minC{mt_minC}_minAF{mt_minAF}/lineage_chrM.ipynb" #"data/vireo/{name}_chrM.ipynb"
    params:
...
rule pseudo_bulk_create_chrM:
    """Subsets and combines the samples to see if it can properly
        deconvolve the two."""
    #TODO
    input: lambda wildcards: expand("data/chrM/{name}_cellSNP_minC{{mt_minC}}_minAF{{mt_minAF}}", name=config["samples"])# , minC=wildcards.minC, minAF=wildcards.minAF)
    params:
        num_cells=num_cells,
        is_prop = is_prop,
        outdir = lambda wildcards, output: dirname(output[0])
    output: "data/chrM/pseudo/minC{mt_minC}_minAF{mt_minAF}/numC{num_cells}_isprop{is_prop}/cellSNP.tag.AD.mtx"
    log: "logs/vireo/chrM/pseudo/minC{mt_minC}_minAF{mt_minAF}/numC{num_cells}_isprop{is_prop}.log"
    shell: "python -m src.pseudo_batch {params.outdir} {input} --num_cells {params.num_cells} --is_prop {params.is_prop} > {log} 2>&1"


rule pseudo_vireo_chrM:
    input:
        AD_F="data/chrM/pseudo/minC{mt_minC}_minAF{mt_minAF}/numC{num_cells}_isprop{is_prop}/cellSNP.tag.AD.mtx",
        notebook=join(ROOT_DIR, "src", "vireo", "vireoSNP_MT_Donors_proper.ipynb" )
    output: "data/chrM/pseudo/minC{mt_minC}_minAF{mt_minAF}/numC{num_cells}_isprop{is_prop}/pseudo.ipynb"
    params:
        INDIR = lambda wildcards, input: dirname(input.AD_F),
        #DP_F=lambda wildcards, input: join(dirname(input.AD_F), "cellSNP.tag.DP.mtx"),
        #VCF_F=lambda wildcards, input: join(dirname(input.AD_F), "cellSNP.base.vcf.gz")
