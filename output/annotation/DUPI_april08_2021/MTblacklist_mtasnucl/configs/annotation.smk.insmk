    ...
    output:
        ...
        report(expand("{{outdir}}/annotation/data/{{prefix}}/mergedSamples/{f}",
                      f=["QC_02.png",
                         ...
                         "integrated.lsi.clusters.png"]))
    params:
        outdir =lambda wildcards, output: dirname(output[0]),
        rscript= join(ROOT_DIR, "R_scripts/annotations/integrateSignac.ipynb"), # The script defaults to the granja data
        sample_names = ",".join(samples.index),
        samples = ",".join(samples["cellr_ID"])
    shell: "papermill -p cellr_in {params.outdir} -p outdir {params.outdir} -p samples {params.samples} -p sample_names {params.sample_names} {params.rscript} {output[0]}"
#######################################################################
